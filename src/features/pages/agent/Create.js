...
import { BetAmount } from "../settings/betAmount/BetAmount";

export default function CreateUser(props) {
  const { userData } = useContext(AppContext);
  var loginUserInfo = JSON.parse(userData);
  ...
  const SaveNewUser = () => {
    //  props.setLoading(true) edited by kyn --20-4-2021-- user create error
    ...
    for (const [key, value] of Object.entries(newuser)) {
      if (
        ...
        isUserError = value == 0 ? true : false;
        if (value == 0) {
          break;
        }
      } else {
        isUserError = value == "" ? true : false;
        if (value == "") {
          break;
        }
      }
    ...
    // props.setLoading(false) edited by kyn --20-4-2021-- user create error
  };
  ...
  const SaveData = () => {
    if (commissionError.com1) {
      ...
      let userData = {
        username:
          loginUserInfo.roleId == 1
            ? "" + newuser["userId"]
            : loginUserInfo.username + newuser["userId"], // Update by Arkar Phone Myat 26.4.21
      ...
      userController.createUserInfoAndCommission(userData, (data) => {
        setIsLoading(false);
  ...
  useEffect(() => {
    // loginUserInfo = JSON.parse(userData);
    if (loginUserInfo) {
      if (loginUserInfo.userId != 1) {
  ...
  const OnChangeNewUser = (text, value) => {
    ...
    if (text == "roleId") {
      ...
      //     ? ""
      //     : "Value must be between " + m_min + " and " + m_max + " !"; /// condition modified by TNA (07/06/2021)
      // }else  newErrorForUser[text] = "";
      if (value == "") {
        newData[text] = value;
      ...
      //         ? ""
      //         : "Value must be between " + s_min + " and " + s_max + " !"; /// condition modified by TNA (07/06/2021)
      //   }else newErrorForUser[text] = "";
      // } else {
    ...
    if (value == "") {
      ...
      if (loginUserInfo.userId != 1) {
        if (value == 0) {
          ...
          // newData[text] = value;
          // newComError[text] = value < logincommission[text] ?
          // "" : `Commission Amount must be less than ${logincommission[text]}!`;

          if (logincommission[text] < value) {
            newComError[
              text
            ] = `Commission Amount can't be greater than ${logincommission[text]}!`;
    ...
    <div>
      ...
      <form autoComplete="off">
        ...
        <div className="col-12 m-0 p-2">
          {!isLoading && (
            <Button title="SAVE" icon="fa fa-save" onClick={SaveNewUser} />
          )}
          {isLoading && (
            <Button
              title="SAVING......"
    ...
    <div className="row">
      <div className="col-md-6 col-12">
        <UserIdTextBox
          ...
          roleId={loginUserInfo && loginUserInfo.roleId} // add by kyn 21-4-2021 remove prefix name in admin
          title="User Id"
          type="text"
          placeholder="User Id"
      ...
      <div className="col-md-6 col-12">
        <NumberTextBox
          ...
          placeholder="0"
          errortext={
            newuser.betLimitForSingle ? newUserError.betLimitForSingle : null
          }
...
export function SelectingBox(props) {
  const { errortext, value, onChange, data, title } = props;
  return (
...
export function UserIdTextBox(props) {
  const {
    defaultTitle,
    roleId, //add by kyn 21-4-2021 remove prefix name in admin
    title,
    type,
    placeholder,
    errortext,
  ...
  //add by kyn 21-4-2021 remove prefix name in admin
  var prefixtitle = roleId == 1 ? "" : defaultTitle;
  return (
...
export function TextBox(props) {
  const { title, type, placeholder, errortext, required, value, onChange } =
    props;
  return (
    <div className="form-group" style={{ marginRight: 5, marginLeft: 5 }}>
      <label
